...
import (
	...
	"github.com/Bunyamin54/funtemps/conv"
)

func ConvertTemperature() {
	overwriteFile := checkFileExists()
	if !overwriteFile {
		fmt.Println("Tilbake til main ")
		return
	}
...
func checkFileExists() bool {
	if _, err := os.Stat("kjevik-temp-fahr-20220318-20230318.csv"); err == nil {
		fmt.Print("Utsikriftsfilen finnes allerede, lage ny? (j/n): ")
...
func createOutputFile() (*os.File, error) { // creates when the program runs
	...
	if _, err := os.Stat(outputFilePath); err == nil {
		fmt.Printf("File %s allerede finnes. Sletting...\n", outputFilePath)
...
func ProcessLine(line string) string {
	...
	if line[0:7] == "Data er" {
		return "Datoen er kanskje ikke oppdatert"
	} else {
...
func AverageTemperature() { //get the acerages
	...
	fmt.Println("Venligst velg (celsius/fahr):") //choose celc or fahr
	var unit string
	fmt.Scan(&unit)
	...
	for i, line := range lines {
		...
		if unit == "fahr" {
			...
			temperature = conv.CelsiusToFarhenheit(temperature) //using funtemps
		}
		sum += temperature
		count++
	}
